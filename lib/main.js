...
module.exports = class NodeLogs {
	...
	// get logs
		...
		lastWritableFile(lastfilenumber) {
			...
			if ("number" !== typeof lastfilenumber || 1 > lastfilenumber) {
				lastfilenumber = 1;
			}
	...
	// formate log
		...
		logInFile(msg, type) {
			...
			return this.lastWritableFile().then((filepath) => {

				let dirname = path.dirname(filepath);

				return fs.isDirectoryProm(dirname).then((exists) => {

					if (exists) {
						return Promise.resolve();
					}
					else {
						return fs.mkdirpProm(dirname);
					}
